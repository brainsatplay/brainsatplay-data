 ...
 * New data system
 * 
 *     User Generates Data e.g.:
 *       - EEG
 *       - Webgazer
 * 
 *     Data is captured in specific structs with the UserPlatform generic struct metadata attached
 *     Biosensing structs are generated from functions and assign whatever info to input objects    
 * 
 *     Data is linked to a DataTablet collection system for parsing it locally and interfacing with 
 *     UserPlatform (UserPlatform is gonna change too so DataTablet gets a bigger role to mediate device data). 
 *       - Move the parsing functions from DataAtlas, 
 *          keep device-specific ones in their own util sets (without a redundant or excessive number of files)
 *       - Keep things functional, don't really need the DataAtlas class anymore for these.
 * 
 *       - Thread friendly, offload BrowserFS to thread, offload raw data parsing to thread
 *          and be able to ask for buffered data from the thread (with rollover on the buffers running and data saved to BrowserFS).
 * 
 * So okay, workflow:
 * 
 * Create data structures
 * 
 * webgazer in this case
 * 
 * let struct = new Struct(
 *   new EyeTrackerStruct(), 
 *   'webgazer', 
 *   undefined, 
 *   userInfo //if using the userplatform
 * );
 * 
 * let threaded = true;
 * let tablet = new DataTablet({webgazer:struct},true); //if threaded, this tablet uses proxies to get/set data from the thread
 * 
 * //Connect device
 * 
 * device.setup = (deviceInfo={blah:'blah',tags:{x:'x',y:'y'}}) => {
 *    //based on deviceInfo and application flags, set up data structures automatically
 *      
 *    //which channel goes to which coordinate right  
 *      Object.keys(deviceInfo.tags).forEach((tag) => {
 *         
 *      });  
 * }
 * 
 ...
 *  //parse data
 * 
 *  //send to data structures
 *   
 * 
 *  //send to algorithms
 *  // should we draw from a settings object that 
 *  //   toggles algos and threads? yes.
 * 
 *  // this can happen on a thread:
 *  //update any tablet references (any static data can get a struct, 
 *  //  e.g. an event like 'device stream started')
 * 
 *  //update anything else (e.g. flagging visualizations, but those should just 
 *  //   use getters on their own loops as we need to account for more complex cases
 *  //     like thread pipelines)
 * 
 * }
 * 
 * 
 * Set up data structures you want for device (smoother than the channel tags object)
 * 
 * Connect device to data structures (either single or multithread toggle)
 * 
 * Connect analysis protocols (can also be multithreaded) which will update device data structures with their unique results (e.g. alphabeta_ratio)
 * 
 * Use handles on the frontend to get what data you want
 * (incl outputting ratios and stuff for post-FFT analysis 
 * which you can chain on to be buffered and saved for use)
 * 
 * Standard ways to pipe data to BrowserFS for saving. And to DataTablet (with rollover)
 */
